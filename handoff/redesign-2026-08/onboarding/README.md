...
> 상태: **확정** (미확정 0 · 재진입 O-12는 기본값 제안) · 근거: Figma `오니기리 칸지` node **298-44943**(섹션, dev) · 화면 PNG 8종(`screens/`, **2026-09 최종 재익스포트**) · 모션 영상 2종(`motion/`)
> 상위 리디자인: [`../README.md`](../README.md) · 토큰 정본 [`../../../DESIGN.md`](../../../DESIGN.md), 컴포넌트 계약 [`../../../COMPONENTS.md`](../../../COMPONENTS.md)가 최종. 값 충돌 시 정본이 이긴다.

## 핵심 요약
앱을 **처음 연 유저**가 학습→재료 획득→영수증까지 **한 사이클을 미리 체험**하는 온보딩. **10단어(O-8)** 짧은 세션이며, **"연습"이라 결과·스트릭·재료는 저장되지 않고**(O-6) 마지막에 실제 오늘 학습으로 진입한다.

- 흐름: **① 시작(사장 인사) → ② 안내(먼저 10개만) → ③④ 학습(앞면→뜻 공개, 평가) → ⑤ 재료 획득(파티클+자동 넘어감) → ⑥ 영수증(출력 모션·연습) → ⑦ 마침(오늘 학습 시작)**
...
| 세션 평가 진행 | 진행바 갱신(`N / 10`, O-8), 다음 단어. **10단어 소진** 시 재료 획득(⑤)으로 |
...
| 2 | ② 안내 — "먼저 10개만." + "짧게 공부하고, 재료를 얻는 흐름을 먼저 연습해보자." (마스코트) · *"?" 주먹밥·teaser 카피 없음(O-13)* | `계속`(잉크) = **buttonAutoFill(M-1)** 2초 자동 진행 · 탭=즉시 → ③ |
...
- **진행바**: 상단 연속 1자 바 + **`N / 10`**(O-8). *(초기안 5단어 → 최종 10단어로 확정, Figma 298-44943.)*
...
| **O-8** ✅ | 세션 단어 수 | **10단어.** 진행바 `N / 10`. (초기안 5단어에서 **최종 10단어로 확정** — Figma 298-44943 정합.) |
...
| ID | 항목 | 변경 |
|---|---|---|
| **N-8** | 온보딩 일러스트 화면 3종 | 신규 — `OnboardingIntro`(사장 인사) · `OnboardingGuide`(먼저 10개만) · `OnboardingFinish`(오늘 학습 시작). COMPONENTS.md 카탈로그 추가 대상. |
...
- **화면**: [`screens/`](./screens) — `01-start` · `02-guide` · `03-study-front` · `04-study-revealed` · `05-ingredient-earned` · `06-receipt-practice` · `06b-receipt-result` · `07-finish`
  - ✅ 에셋 최신화(2026-09): `01·02·05·06-receipt·07`을 최종 Figma(298-44943)에서 재익스포트 — **잉크 CTA(O-9)** · **10단어(O-8)** · **② 안내 "?"·teaser 제거(O-13)** · 신규 일러스트(사장/독서 고양이) 반영. `03·04·06b`는 최종과 동일해 유지.
- **모션**: [`motion/`](./motion)
